...
_sv_unsupported_verification_clause() {
  ...
  [[ "$clause" =~ (^|[^[:alnum:]_])(cannot|can[[:space:]]+not|could[[:space:]]+not)[[:space:]]+(verify|confirm|claim)([^[:alnum:]_]|$) ]] \
    ...
    || [[ "$clause" =~ (^|[^[:alnum:]_])(assuming|if[[:space:]]+(all[[:space:]]+)?(the[[:space:]]+)?(build[[:space:]]+(succeeds|pass(es|ed)?)|tests?[[:space:]]+(are[[:space:]]+)?passing|tests?[[:space:]]+pass(ed|ing)?|lint[[:space:]]+pass(es|ed)?|typecheck[[:space:]]+pass(es|ed)?|checks[[:space:]]+pass(ed|ing)?)|when[[:space:]]+(all[[:space:]]+)?(the[[:space:]]+)?(build[[:space:]]+(succeeds|pass(es|ed)?)|tests?[[:space:]]+(are[[:space:]]+)?passing|tests?[[:space:]]+pass(ed|ing)?|lint[[:space:]]+pass(es|ed)?|typecheck[[:space:]]+pass(es|ed)?|checks[[:space:]]+pass(ed|ing)?)|if[[:space:]]+verified)([^[:alnum:]_]|$) ]] \
    || [[ "$clause" =~ (tests?[[:space:]]+pass(ed|ing)?|the[[:space:]]+tests[[:space:]]+(are[[:space:]]+)?passing|the[[:space:]]+test[[:space:]]+passed|verified|build[[:space:]]+(succeeds|pass(es|ed)?)|lint[[:space:]]+pass(es|ed)?|typecheck[[:space:]]+pass(es|ed)?|checks[[:space:]]+pass(ed|ing)?|ran[[:space:]]+(the[[:space:]]+)?tests|confirmed[[:space:]]+working)[[:space:]]*\? ]]
}
...
while IFS= read -r _sv_clause || [[ -n "$_sv_clause" ]]; do
  ...
  if _sv_positive_verification_claim "$_sv_clause" && ! _sv_unsupported_verification_clause "$_sv_clause" && ! _sv_questioned_verification_claim "$_sv_clause"; then
