...
if [[ "${CLAUDE_GUARD_DISABLED:-0}" == "1" || "${CLAUDE_CONTROL_PLANE_RATE_LIMITER:-1}" == "0" ]]; then
  exit 0
fi
...
RAPID_WINDOW="${CLAUDE_CONTROL_PLANE_RATE_LIMITER_WINDOW_SEC:-60}"
RAPID_THRESHOLD="${CLAUDE_CONTROL_PLANE_RATE_LIMITER_RAPID_THRESHOLD:-15}"
FAILURE_THRESHOLD="${CLAUDE_CONTROL_PLANE_RATE_LIMITER_FAILURE_THRESHOLD:-3}"
MAX_LINES="${CLAUDE_CONTROL_PLANE_RATE_LIMITER_MAX_LINES:-50}"
WARN_INTERVAL="${CLAUDE_CONTROL_PLANE_RATE_LIMITER_WARN_INTERVAL_SEC:-60}"
LOCK_TIMEOUT="${CLAUDE_CONTROL_PLANE_RATE_LIMITER_LOCK_TIMEOUT_SEC:-2}"
...
root="${CLAUDE_CONTROL_PLANE_RATE_LIMITER_DIR:-${TMPDIR:-/tmp}/claude-control-plane-rate-limiter}"
...
now="$(date +%s)"
tool_name="$(jq -r '.tool_name // .toolName // .tool // "unknown"' <<<"$HOOK_INPUT" 2>/dev/null || printf 'unknown')"
...
tmp="$(mktemp "${counter}.tmp.XXXXXX")" || exit 0
