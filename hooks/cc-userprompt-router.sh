...
if [[ "$prompt_lower" =~ (^|[^a-z])(((^|[^[:alnum:]_/])agents\.md($|[[:space:]/])|(^|[^[:alnum:]_/])agent\.md($|[[:space:]/])|agents?[[:space:]-]+files?|instruction[[:space:]]+files|startup[[:space:]]+guidance|align[[:space:]]+.*agents\.md|align[[:space:]]+.*claude\.md|prune[[:space:]]+(agents|claude|rules)|rule[[:space:]-]?bloat|agents\.md[[:space:]]+(too[[:space:]]+long|too[[:space:]]+big|bloated)|claude\.md[[:space:]]+(too[[:space:]]+long|too[[:space:]]+big|bloated)|startup[[:space:]]+(file|context)[[:space:]]+(too[[:space:]]+long|bloated)|trim[[:space:]]+(agents|claude)\.md))([^a-z]|$) ]]; then
