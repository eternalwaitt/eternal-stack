...
if [[ "$prompt_lower" =~ /email-triage|email[[:space:]-]+triage ]]; then
  ...
  if [[ -n "$email_triage_account" ]]; then
    notes+=("Use /email-triage as two phases. Phase 1: Inbox Zero first. Run: etrnl-email triage guarded-run --account $email_triage_account --max-inbox 500 --apply --require-insights, then verify with etrnl-email triage verify --latest --account $email_triage_account. Do not open the queue unless verify reports inbox_zero_verified true, inbox_count 0, and either gmail_mutated true or queue_ready_without_mutation true. Phase 2: only after Inbox Zero, paste one generated queue item with etrnl-email triage queue --run-id <run-id> --mode reply --format markdown --next. Do not say triage complete while an item is active.")
  else
    notes+=("Use /email-triage as two phases. If no account id is present, ask for it. Phase 1: Inbox Zero first with etrnl-email triage guarded-run --account <account-id> --max-inbox 500 --apply --require-insights, then etrnl-email triage verify --latest --account <account-id>. Do not open the queue unless verify reports inbox_zero_verified true, inbox_count 0, and either gmail_mutated true or queue_ready_without_mutation true. Phase 2: only after Inbox Zero, paste one generated queue item with etrnl-email triage queue --run-id <run-id> --mode reply --format markdown --next. Do not say triage complete while an item is active.")
  fi
fi
