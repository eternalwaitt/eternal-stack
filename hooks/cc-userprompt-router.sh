...
imperative_execution_pattern='(^|[[:space:][:punct:]])(implement|execute|fix|edit|write|commit|ship|refactor|apply|patch|deploy|carry[[:space:]]+out|run[[:space:]]+the[[:space:]]+(plan|tests|checks)|do[[:space:]]+it|go[[:space:]]+ahead)([[:space:][:punct:]]|$)|(^|[[:space:][:punct:]])(build|update|change|modify|create|add|remove|delete)[[:space:]]+(the|this|my|our|a|an|it|that|code|file|files|config|setting|settings|project|app|service|api|test|tests|plan|task|tasks|branch|pr|repo|repository|module|component|function|class|method|schema|migration|package|hook|hooks|skill|skills|rule|rules|doc|docs|readme|changelog|version|release|deployment|feature|bug|fix|issue|ticket)([[:space:][:punct:]]|$)'
execution_intent_pattern='(^|[[:space:][:punct:]])(implement|execute|fix|edit|write|commit|ship|refactor|migrat(e|ion)|apply|patch|deploy|carry[[:space:]]+out|run[[:space:]]+(the[[:space:]]+)?(tests|checks|test[[:space:]]+suite)|run[[:space:]]+the[[:space:]]+plan|implement[[:space:]]+the[[:space:]]+plan|execute[[:space:]]+the[[:space:]]+plan|continue[[:space:]]+the[[:space:]]+plan|finish[[:space:]]+the[[:space:]]+plan|resume[[:space:]]+the[[:space:]]+plan)([[:space:][:punct:]]|$)|(^|[[:space:][:punct:]])(build|update|change|modify|create|add|remove|delete)[[:space:]]+(the|this|my|our|a|an|it|that|code|file|files|config|setting|settings|project|app|service|api|test|tests|plan|task|tasks|branch|pr|repo|repository|module|component|function|class|method|schema|migration|package|hook|hooks|skill|skills|rule|rules|doc|docs|readme|changelog|version|release|deployment|feature|bug|fix|issue|ticket)([[:space:][:punct:]]|$)|(^|[[:space:]])(do[[:space:]]+it|go[[:space:]]+ahead)([[:space:][:punct:]]|$)'
...
if [[ "$prompt_lower" =~ $read_only_question_pattern ]] && [[ ! "$prompt_lower" =~ $imperative_execution_pattern ]]; then
  cc_state_update '.planExecutionRequested = false | .planExecutionRequestedAt = ""' >/dev/null || true
fi
...
if [[ "$prompt_lower" =~ (^|[^a-z])((agents?\.md|agents?[[:space:]-]?files?|instruction[[:space:]]+files|startup[[:space:]]+guidance|align[[:space:]]+.*agents\.md|align[[:space:]]+.*claude\.md|prune[[:space:]]+(agents|claude|rules)|rule[[:space:]-]?bloat|agents\.md[[:space:]]+(too[[:space:]]+long|too[[:space:]]+big|bloated)|claude\.md[[:space:]]+(too[[:space:]]+long|too[[:space:]]+big|bloated)|startup[[:space:]]+(file|context)[[:space:]]+(too[[:space:]]+long|bloated)|trim[[:space:]]+(agents|claude)\.md))([^a-z]|$) ]]; then
