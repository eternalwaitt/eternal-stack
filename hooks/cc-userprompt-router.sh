...
if [[ "$prompt_lower" =~ /email-triage|email[[:space:]-]+triage ]]; then
  ...
  if [[ -n "$email_triage_account" ]]; then
    notes+=("Use /email-triage as two phases. Phase 1: Inbox Zero first. Run: vivaz-email triage guarded-run --account $email_triage_account --max-inbox 500 --apply --allow-apply-before-enrichment --progress, then verify with vivaz-email triage verify --latest --account $email_triage_account. Do not set a Bash tool timeout on guarded-run. Do not start a second guarded-run until verify succeeds or the first run recorded failure. Do not open the queue unless verify reports inbox_zero_verified true and inbox_count 0. Phase 2: only after Inbox Zero, paste one generated queue item with vivaz-email triage queue --run-id <run-id> --account $email_triage_account --mode review --format markdown --next. Drafts generate on demand for the current item only. Do not say triage complete while an item is active.")
  else
    notes+=("Use /email-triage as two phases. If no account id is present, ask for it. Phase 1: Inbox Zero first with vivaz-email triage guarded-run --account <id> --max-inbox 500 --apply --allow-apply-before-enrichment --progress, then vivaz-email triage verify --latest --account <id>. Do not set a Bash tool timeout on guarded-run. Do not start a second guarded-run until verify succeeds or the first run recorded failure. Do not open the queue unless verify reports inbox_zero_verified true and inbox_count 0. Phase 2: only after Inbox Zero, paste one generated queue item with vivaz-email triage queue --run-id <run-id> --account <id> --mode review --format markdown --next. Drafts generate on demand for the current item only. Do not say triage complete while an item is active.")
  fi
fi
...
if [[ "$prompt_lower" =~ (^|[^a-z])(((^|[^[:alnum:]_/])agents\.md($|[[:space:]/])|(^|[^[:alnum:]_/])agent\.md($|[[:space:]/])|agents?[[:space:]-]+files?|instruction[[:space:]]+files|startup[[:space:]]+guidance|align[[:space:]]+.*agents\.md|align[[:space:]]+.*claude\.md|prune[[:space:]]+(agents|claude|rules)|rule[[:space:]-]?bloat|agents\.md[[:space:]]+(too[[:space:]]+long|too[[:space:]]+big|bloated)|claude\.md[[:space:]]+(too[[:space:]]+long|too[[:space:]]+big|bloated)|startup[[:space:]]+(file|context)[[:space:]]+(too[[:space:]]+long|bloated)|trim[[:space:]]+(agents|claude)\.md|review[[:space:]]+agents\.md\.($|[[:space:]])|update[[:space:]]+agent\.md\?($|[[:space:]])))([^a-z]|$) ]]; then
