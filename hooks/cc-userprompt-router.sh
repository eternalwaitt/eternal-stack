...
execution_intent_pattern='(^|[[:space:][:punct:]])(implement|execute|fix|edit|write|commit|ship|refactor|migrat(e|ion)|apply|patch|deploy|build|create|add|remove|delete|update|change|modify|carry[[:space:]]+out|run[[:space:]]+(the[[:space:]]+)?(tests|checks|test[[:space:]]+suite)|run[[:space:]]+the[[:space:]]+plan|implement[[:space:]]+the[[:space:]]+plan|execute[[:space:]]+the[[:space:]]+plan|continue[[:space:]]+the[[:space:]]+plan|finish[[:space:]]+the[[:space:]]+plan|resume[[:space:]]+the[[:space:]]+plan)([[:space:][:punct:]]|$)|(^|[[:space:]])(do[[:space:]]+it|go[[:space:]]+ahead)([[:space:][:punct:]]|$)'
