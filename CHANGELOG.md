...
- The stale-lock reclaim path can no longer hand one lock to two processes. A reclaim marker excludes rival reclaimers, and an owner-token comparison refuses the delete when the real owner released and a new acquirer replaced the lock mid-reclaim — previously a reclaimer that had judged a lock dead could delete the fresh lock that replaced it and then enter the critical section alongside its holder. A non-numeric `ETRNL_LEDGER_LOCK_TIMEOUT_MS` no longer spins the acquire loop forever either: `Number("abc")` is `NaN`, which compares false against every elapsed time, so a typo disabled both the timeout and the staleness reclaim instead of failing fast.
