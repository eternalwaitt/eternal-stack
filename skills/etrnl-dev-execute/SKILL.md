...
After the final edit of a task or wave, run `node ~/.claude/scripts/review-rules.mjs check --changed-only` whenever the tree has source changes, then run parallel reviewers, merge with `node ~/.claude/scripts/review-merge.mjs`, fix `safe_auto` immediately, and reopen only on P0/P1 blockers. Load `references/bounded-review.md` for synthesis, reopen caps (ledger-enforced), and per-tier depth.
