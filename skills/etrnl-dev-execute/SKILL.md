...
3. Group tasks by dependency and write scope. Execute dependent work sequentially; dispatch independent read-only review or disjoint write work to fresh subagents. For explicit parallel fan-out requests, load `references/parallel-fanout.md` before widening lanes.
   ...
   - **Before every subagent dispatch on Claude or Codex**, run `node ~/.claude/scripts/execution-ledger.mjs check-spawn --session "$CLAUDE_SESSION_ID" --task-name "<name>" --wave "<wave-or-phase>"`. When `cc-spawn-guard.sh` or the Codex spawn hook is active, use `--dry-run` in the skill layer only — the hook records spawns. A non-zero exit is a hard stop — fix the wave shape or record the required decision; never rename spawns to bypass the guard. Load `references/claude-execute-profile.md` or `references/codex-execute-profile.md` for host-specific lane caps and hook contracts.
    ...
    - A user reply such as "continue", "all at once", or "don't ask for approvals" means **skip mid-run approval prompts**, not skip batching, spawn-guard checks, review caps, or ledger gates. Still record `batch-execution-adopted` when batching applies and still run `check-spawn` before every subagent.
