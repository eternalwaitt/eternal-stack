...
2. Separate lanes:
   - Local hooks: cheap deterministic checks that block obvious bad pushes. Install the CodeRabbit-preemption guard as a pre-push step: `node scripts/review-rules.mjs check --base <upstream-ref>` — scope to the outgoing range `<upstream-ref>...HEAD` (the ref the pre-push hook receives on stdin), not the clean worktree, and it fails closed on an unresolvable ref. (`--changed-only` is the working-tree variant for pre-commit/local runs.)
