...
Run `node scripts/review-rules.mjs check --base <upstream-ref>` in a pre-push hook (engines: ast-grep + literal; block/warn modes; rules in `review-rules.json`). Use `--base` — the outgoing range `<upstream-ref>...HEAD` (e.g. the remote ref the hook receives) — for pre-push, because at push time the changes are already committed and the working tree is clean, so `--changed-only` (working-tree scope, for pre-commit or local runs) would see nothing. `--base` fails closed if the ref cannot be resolved, so a misconfigured hook errors instead of silently checking zero files. SaaS-on-`eternal-saas` repos can additionally enable the opt-in overlay `templates/review-rules.saas.example.json` (`--config` that file): warn-mode guards for React-Compiler manual memo (`useCallback`/`useMemo`) and unchecked `searchParams.get(...) as T` casts — the highest-frequency mechanically-catchable classes from the mined corpus, held in warn until the learning loop escalates them. These are the "quick win" tail CodeRabbit posts every PR:
