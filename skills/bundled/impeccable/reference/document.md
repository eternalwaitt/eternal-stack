...
If a `DESIGN.md` already exists, **do not silently overwrite it**. Show the user the existing file, STOP, and ask whether to refresh, overwrite, or merge — use Codex's structured user-input/question tool when available; if unavailable, ask directly in chat.
