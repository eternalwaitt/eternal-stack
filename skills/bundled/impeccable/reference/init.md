...
## Step 6: Live mode (not available in this vendored copy)

Upstream Impeccable pre-configures a live browser variant mode here. That mode depends on upstream's executable helper stack, which is not included in this vendored copy — skip this step entirely; do not write `.impeccable/live/config.json` or patch CSP. See [live.md](live.md) for the manual iteration alternative.
...
- **Iterate visually** (web only): iterate manually — edit source, preview in the browser, and use `polish` / `layout` / `animate` (upstream's `$impeccable live` variant mode is not included in this vendored copy). **Skip this group for native platforms.**
