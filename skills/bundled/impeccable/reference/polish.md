...
4. **Pull in any prior critique** (optional signal): If `$impeccable critique` has been run on the same target, its priority issues are a useful prior for what to address first. Derive the target's slug (lowercase, non-alphanumerics collapsed to `-`) and look for the most recent matching snapshot file under `.impeccable/critique/`. If one exists, fold its P0/P1 items into your polish list and mention the snapshot path so the user sees what you read. If none exists, continue without it. The critique is one input among many. Do your own pass either way.
