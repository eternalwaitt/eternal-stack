...
| `wcag-accessibility` (bundled) | ux-04 | Load for criterion-level depth: map each ux-04 finding to a WCAG 2.1/2.2 success criterion and level (A/AA). Without it, record heuristic observations but mark affected ux-04 coverage `source_limited`; do not report criterion-level coverage as confirmed clean. |
