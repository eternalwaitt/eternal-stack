...
- Background `{colors.canvas}`, padding `{spacing.xxl}`, rounded `{rounded.lg}` 12px, 1px `{colors.hairline}` border, Level 1 shadow when elevation is required.
...
### Do
- Reserve `{colors.primary}` for filled CTAs and inline link emphasis — it appears sparingly, one filled button per band.
