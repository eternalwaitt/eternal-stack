<!-- From VoltAgent/awesome-design-md@664b3e78 (MIT) -->
...
| `{typography.heading-1}` | 48px | 600 | 1.15 | -0.5px | Page-level headlines ("Get started free") |
...
- Background `{colors.surface}`, text `{colors.ink}`, typography `{typography.body-sm-medium}`, padding `{spacing.sm} {spacing.md}`. ("Developers: Get a first look at our new Developer Platform on 2024-05-13.")
...
- Animation/transition timings not extracted; use 150–200ms ease
- Form validation success state not explicitly captured
- Pastel-tint mapping (which feature uses which tint) is observation-based — the actual brand library can include more entries
