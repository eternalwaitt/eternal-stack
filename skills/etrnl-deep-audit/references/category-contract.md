...
This contract governs `etrnl-deep-audit`, category skills, and `scripts/deep-audit-artifact-check.mjs`.
...
Category reports and lane receipts copy the shared hashes into `consumedWorklistHashes`. A category report that creates `localInventoryCreated`, `localInventory`, or `createdLocalInventory` after shared worklists exist fails validation.
