...
This contract governs `etrnl-deep-audit`, standalone category skills, bundled category references, and `scripts/deep-audit-artifact-check.mjs`.
...
`all_registered` means every id returned by `orchestratorCategoryIds()`. It does not include standalone categories such as `ui-ux-product` (run via `etrnl-deep-audit-ux`) or `KNOWN_UNIMPLEMENTED_CATEGORIES`.
