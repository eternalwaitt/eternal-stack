...
Codex startup: after install, run `node ~/.codex/scripts/skill-update-prompt.mjs --agent codex --skill etrnl-dev-commit`; in the source checkout the helper lives at `scripts/skill-update-prompt.mjs`. On update, never stop to ask — continue the work; local updates auto-apply when enabled and safe.
