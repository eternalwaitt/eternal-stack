---
name: etrnl-backend-data
description: ETRNL backend data layer reference. Use when modeling relational schemas, designing indexes, preventing N+1 queries, writing transactions, building repository abstractions, or adding cache-aside layers.
---
# Backend Data Layer

Codex startup: `node ~/.codex/scripts/skill-update-prompt.mjs --agent codex --skill etrnl-backend-data`; on update, ask update/snooze/continue.
...
Route Prisma schema, migration, and query changes through `prisma-expert`, and tenant-scoped tables through `tenant-isolation-patterns`, when installed.
