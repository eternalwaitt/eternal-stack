...
4. Apply the loaded references to the user's task. Pull companion skills (`eternal-best-practices`, `better-auth`, `tenant-isolation-patterns`, legacy `orpc-patterns`, legacy `prisma-expert`, legacy `sql-optimization-patterns`) only when the task crosses boundaries this repo does not already cover.
