...
mkdir -p "$TARGET/hooks" "$TARGET/scripts" "$TARGET/docs/templates" "$TARGET/skills" "$TARGET/rules"
...
chmod +x "$TARGET/hooks/test-hooks.sh" "$TARGET/scripts/"*.sh "$TARGET/scripts/code-health-inventory.mjs" "$TARGET/scripts/plan-readiness-check.mjs"
