...
if [[ -f "$BACKUP/new-source-paths.txt" ]]; then
  while IFS= read -r new_rel; do
    ...
    case "$new_rel" in
      hooks/* | tests/fixtures | tests/lib/* | tests/test-hooks.sh | tests/test-workflow-tools.sh | rules-manifest.json | docs/templates/rules/eternal-saas | docs/templates/rules/eternal-saas/*) : ;;
      *) continue ;;
    esac
    case "$new_rel" in
      *..*) continue ;;
