...
function commandReferencesPath(commandText, itemPath) {
  ...
  return commandString.includes(pathString) || commandString.includes(pathString.replace(/ /g, "\\ "));
}
...
function validateManifest(manifest) {
  ...
  for (const [index, item] of (items || []).entries()) {
    ...
    if (item.riskTier >= 2 && !item.requiresApproval) errors.push(`items[${index}].requiresApproval is required for risk tier 2 or 3`);
  }
  return errors;
}
