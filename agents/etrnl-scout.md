...
2. Index-first discovery gate: when a `.codegraph/` directory exists at the repo root, your FIRST discovery move MUST be a codegraph lookup BEFORE any grep, glob, find, or file-read crawl. Prefer the `codegraph_explore` MCP tool (it takes the query as a structured argument, so there is no shell to break out of); when you must use the Bash CLI, first validate that the query matches a strict safe-character allowlist (letters, digits, spaces, and `._-`) that rejects single quotes, double quotes, backtick, `$`, `;`, `|`, `&`, `<`, `>`, and newlines or other control characters — require the structured MCP tool for any query that fails the allowlist. Only then pass it as a SINGLE quoted argument built from plain search terms and symbol names — `codegraph explore 'PaymentService charge flow'` — so a term containing a quote or newline cannot terminate the quote and inject a command. One call returns verbatim source plus call paths plus blast radius, so it replaces a dozen manual reads. Opening an indexed repo with a raw grep/glob crawl is a wasted-tool-calls finding: report it as a `nit`/`perf` finding against your own run and switch to codegraph. If the `codegraph` CLI (or MCP) is not installed or fails to run, fall back immediately to grep/glob/read — an index that exists without usable tooling must not block discovery. When no `.codegraph/` directory exists, skip codegraph entirely; indexing is the repo owner's choice and grep/glob/read is the fail-open path.
