...
2. Enumerate consumers. Prefer CodeGraph when a `.codegraph/` index exists: use the `codegraph_explore` MCP tool (it takes the query as a structured argument, so there is no shell to break out of), or when using the Bash CLI, first validate that every symbol matches a strict safe-character allowlist (letters, digits, spaces, and `._-`) that rejects single quotes, double quotes, backtick, `$`, `;`, `|`, `&`, `<`, `>`, and newlines or other control characters — otherwise require the structured `codegraph_explore` MCP tool. Only after that check pass the query as a SINGLE quoted argument built from plain symbol names — `codegraph explore 'PaymentService callers and references'` — so a symbol containing a quote or newline cannot terminate the quote and inject a command. CodeGraph returns call paths including dynamic-dispatch hops grep cannot follow. Fall back to `rg`/`sg` for text and structural matches. If the CodeGraph MCP tool and CLI are both unavailable or fail to run — or a symbol falls outside the Bash allowlist and the structured `codegraph_explore` MCP tool cannot be used — do NOT block: fall back immediately to `rg`/`sg` (which take the symbol as a plain argument, so an unsafe symbol never reaches a shell) and note in your output that dynamic-dispatch hops CodeGraph would catch may be missed. An unsafe symbol must never be routed through the `codegraph` Bash CLI; enumeration must stay fail-open. Include re-exports, destructured reads, and JSX prop threads.
