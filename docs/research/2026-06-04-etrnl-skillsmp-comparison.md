...
- Under-enforced behavior: `etrnl-systematic-debugging`, `etrnl-pr`, and `etrnl-commit` rely on prose only and lack helper-level preflight; quick wins can strengthen directive text, helper scripts belong in P1.
- Prompt-budget risks: `etrnl-execute`, `etrnl-autoplan`, and `etrnl-plan` are already large. Additions there must stay short; move extended examples to references or validators.
- Category coverage gap: `etrnl-deep-audit` registers only `production-readiness` and `performance`; security, UX/accessibility, API/data, docs, payments, and privacy/compliance remain known-unimplemented.
...
- `skills/etrnl-systematic-debugging/SKILL.md`: rename from fix-issue and expand into a compact systematic debugging contract with untrusted issue guard, eligibility stops, root-cause phases, instrumentation, failed-fix escalation, focused tests, review loop, and no remote side effects without approval.
- `skills/etrnl-pr/SKILL.md`: add branch/auth/upstream/existing-PR preflight, CI evidence, terminal conditions, and no merge/force-push/comment side effects unless requested.
- `skills/etrnl-commit/SKILL.md`: add atomic grouping, exclusions, commit-style detection, post-commit verification, and hook-failure handling.
- `skills/etrnl-deps/SKILL.md`: add audit/upgrade/bot-pr modes, major confirmation, current docs/changelog/source-diff, related package detection, and no force-audit-fix/manual lockfile edits.
- `skills/etrnl-brainstorm/SKILL.md`: add spec self-review checklist.
- `skills/etrnl-plan/SKILL.md` and `skills/etrnl-autoplan/SKILL.md`: add compact vertical-slice/task-sizing guard.
- `skills/etrnl-review/SKILL.md`: add tests-first, dependency discipline, and change-size lens.
- `skills/etrnl-test/SKILL.md`: add behavior inventory, positive/negative cases, and deterministic test acceptance.
- `skills/etrnl-qa-browser/SKILL.md`: add tool resolution, snapshot/state before interactions, named sessions, and cleanup.
- `skills/etrnl-stress-test/SKILL.md`: clarify load/stress testing authorization and threshold contract.
- `skills/etrnl-disk-cleanup/SKILL.md`: add manifest schema, risk tiers, owner cleanup commands, and no whole-Trash emptying.
- `skills/etrnl-email-reply-quality/SKILL.md`: add English AI-tell blocks and detect/rewrite/self-check wording.
- `skills/etrnl-agent-files/SKILL.md`: add compact scorecard and untrusted repo text rule.
- `skills/etrnl-context-save/SKILL.md` and `skills/etrnl-context-restore/SKILL.md`: add typed context taxonomy and skip rules.
...
- DONE: Add execution wave drift checks through a helper or validator-backed contract rather than enlarging `etrnl-execute`.
...
- REJECTED_FOR_NOW: Split `etrnl-review` into tiny sibling lenses only if prompt routing shows repeated overload or prompt-budget pressure. Current evidence does not show repeated overload, so the single skill keeps a split-lens trigger instead of adding new prompt surfaces.
