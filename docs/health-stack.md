...
- `deep-audit-artifact-check.mjs` is the source gate for registered deep-audit category artifacts. It validates category registry alignment, all registered check ids, lane-specific worklist hashes, private-string redaction, coverage statements, and problem/cause/fix diagnostics. Performance reports also preserve provider incident ids, reject clean/source-limited concealment and build/client metric substitution, and require affected-journey runtime closure evidence.
