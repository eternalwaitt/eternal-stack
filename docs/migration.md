...
6. Confirm installed scripts, docs, agents, settings, and mode with `scripts/doctor.sh` and `~/.claude/scripts/doctor-control-plane.sh`.
7. Confirm local ledger and artifact helpers report cleanly with `scripts/workflow-health.mjs`.
8. Hard blockers one group at a time with `CLAUDE_CONTROL_PLANE_ENABLE_STRICT=1`, including `PreToolUse`, `PostToolUseFailure`, `Stop`, and `SubagentStop`.
