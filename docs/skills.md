...
| `review-merge.mjs` | `~/.claude/scripts/review-merge.mjs` | Merges parallel reviewer findings JSON into one artifact: fingerprint dedup with cross-reviewer confidence boost, confidence gates with reported (never silent) drops, and `autofix_class` routing into `blocking` (P0/P1), `safe_auto`, and `residual`; exit 1 only on blocking findings. Canonical review synthesis step in `references/bounded-review.md`. |
