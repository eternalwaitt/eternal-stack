...
- **Execute host profile.** `ETRNL_EXECUTE_HOST=codex` selects a lighter process, `claude` forces the unchanged Claude path, and leaving it unset auto-detects a Codex session. The Codex profile drops `maxConcurrentLanes` to 2 and merges review into one pass per wave for tier 0–2. Tier 3 keeps `etrnl-spec-reviewer`, `etrnl-quality-reviewer`, and the simplifier lens, running the per-task chain on wave 1 and one merged review per wave from wave 2 unless the plan names full fan-out for that wave. Tier-3 gates — staged install proof, rollback proof, reopen caps, consumer tracing, and the auth/money/tenancy/migration lenses — hold at full strength under either profile.
...
| `review-merge.mjs` | `~/.claude/scripts/review-merge.mjs` | `merge` folds parallel reviewer findings JSON into one artifact: fingerprint dedup with cross-reviewer confidence boost, confidence gates with reported (never silent) drops, and `autofix_class` routing into `blocking` (P0/P1), `safe_auto`, and `residual`; exit 1 only on blocking findings. It also owns the review economy: named park thresholds on `recurringFindingCount` (3), `streamAlternationCount` (4), and `roundsSinceProgress` (2), each overridable by env, park a stream before the reopen cap is exhausted; `skip-plan` proposes skipping a reviewer after five consecutive zero-finding dispatches, counting them in an additive `reviewerDispatches` key inside the shared `review-learnings.json`. Security lenses, tenancy lenses, and every deep-audit lane are exempt, with the lane list derived live from the deep-audit registry so new categories are covered automatically and an unloadable registry reports `skipEvaluation: unavailable` and skips nothing. Every skip carries a machine-readable `reasonCode`. Canonical review synthesis step in `references/bounded-review.md`. |
