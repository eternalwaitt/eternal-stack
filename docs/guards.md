...
- `cc-userprompt-router.sh`: records requested skills, reinjects global/project `CLAUDE.md` context, and injects short routing reminders.
...
Strict mode registers `PreToolUse`, `Stop`, and `SubagentStop` blockers. Default mode keeps observer hooks and the advisory rate limiter active while leaving hard blockers opt-in.
